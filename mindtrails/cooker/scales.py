...
class Scale:
    ...
    def trans(self):
        if self.state == 'scored':
            data = pd.pivot_table(self.dataset, index="participantRSA", columns="session")
...
class OA(Scale):
    ...
    def score(self):
        ...
        if self.state == 'raw':
            self.dataset['SUM'] = self.dataset[col_list].sum(axis=1)
            self.state = 'scored'
        return self
...
class DASS21_DS(Scale):
    def __init__(self,dataset,state):
        Scale.__init__(self,dataset,state)
    def score(self):
        col_list=['blue','difficult','meaningless','nervous','noenthusiastic','nopositive','noworth']
        if self.state == 'raw':
            self.dataset['SUM'] = self.dataset[col_list].sum(axis=1)
            self.state = 'scored'
        return self
